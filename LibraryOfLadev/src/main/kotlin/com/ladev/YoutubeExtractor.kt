...
/**
 * YouTube stream extractor using NewPipeExtractor.
 *
 * Direct port of re-3rabi's Yextractor.kt:
 * 1. NewPipeExtractor fetches the YouTube page and extracts all stream URLs
 *    (handles signature decryption, poToken, etc. automatically)
 * 2. Video-only and audio-only streams are separated
 * 3. Audio streams are grouped by language
 * 4. For each video resolution x audio language, a DASH MPD manifest is built
 * 5. Manifests are served via a local HTTP server on a random port
 * 6. The player opens the local MPD URL and streams directly from YouTube's CDN
 */
open class YoutubeExtractor : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        if (ytVideos[videoId].isNullOrEmpty()) {
            try {
                ...
                // ── Cache subtitles from NewPipeExtractor ──
                try {
                ...
                // ── Build DASH manifests: video + best audio per language ──
                ...
                // ── Also emit muxed (video+audio combined) streams as legacy fallback ──
    ...
    // ═══════════════════════════════════════════════════════════════
    //  LOCAL HTTP SERVER FOR DASH MANIFESTS
    // ═══════════════════════════════════════════════════════════════
